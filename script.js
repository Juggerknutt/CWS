...
nextBtn.addEventListener('click', function(event){
    ...
    i++
    counterElement.innerText = `Step: ${[i]} of ${countryCategories.length}`
})
...
prevBtn.addEventListener('click', function(event){
    ...
    counterElement.innerText = `Step: ${[i + 1]} of ${countryCategories.length}`
})
...
searchBtn.addEventListener('click', function(event){
    ...
    displayCountry.append(pElement)
    for(const [key, value] of Object.entries(values[0])) {
        let listElement = document.createElement('li')
        listElement.innerHTML = [`${key}: ${value}`]
        displayCountry.append(listElement)
    }
